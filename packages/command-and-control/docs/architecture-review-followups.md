...
| P2 | Canvas Backup deep import paths verified | **Done** | `canvas-design-mcp/dist/tools/import-course.js` and `generate-course.js` confirmed correct. No `exports` map so deep imports work; Design Studio must be built (`npm run build`) before C&C can import it. Document in build order. *(Package later renamed to `@canvas-toolchain/canvas-design-studio` in v2.2.0)* |
