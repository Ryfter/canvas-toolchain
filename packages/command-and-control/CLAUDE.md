...
- `discover_tools` MCP tool — read-only post-install tool discovery (#76). Best-effort Canvas scan cascade (account → per-course → self-report), matched against the `data/known-tools.yaml` catalog and module `handles[]`. Returns detected tools, module-enable suggestions, unmatched (free-form #77 signal), and a catalog pick-list. Pairs with `set_module_enabled` to enable suggested modules.
...
- `browse_module_catalog` MCP tool — read-only. Fetches `module-catalog.json` (5s timeout, 24h cache) and merges it with local state, reporting each catalog module as bundled / not installed / installed (enabled or disabled) / update available, plus any pending installer-GUI requests (`clearPending: true` discards stale ones). Feeds `discover_tools`' catalog-suggestion matching via `matchCatalogSuggestions`.
- `install_module` MCP tool — two-call confirm gate (same idiom as `wave_deep_check`): call 1 previews name/version/description/size/source/sha256/handles with no side effects (too-old host → `HOST_TOO_OLD` refusal instead of a preview); call 2 (`confirm: true`) downloads to a temp file, verifies its sha256 against the catalog entry (refusing on any mismatch — the single fail-closed choke point in `src/channel/install.ts`), places it atomically, records it in `installed-modules.json`, enables it, and clears any matching pending request. Upgrades retain the previous version's artifact + hash until the new version loads successfully once.
- `uninstall_module` MCP tool — removes a channel-installed artifact + its `installed-modules.json` record and disables it in `modules.json`. Bundled modules (present in `KNOWN_MODULES`) refuse uninstall — `set_module_enabled` is the only lever for those.
- Loader phase 2 (`src/modules/registry.ts` `loadInstalledArtifacts()`/`loadModules()`): after the static `KNOWN_MODULES` pass, reads `installed-modules.json` and, for each enabled entry, **re-hashes the artifact file against its recorded sha256 immediately before every dynamic import** — mismatch, missing file, contract violation, or a throwing import are all skipped with a logged warning, never loaded (fail-closed), and never crash the host (fail-soft; the server always starts). On an id collision between bundled and installed, the **semver-newer version wins** (equal versions keep bundled — no download needed); a successful load prunes any retained previous version. See `docs/module-channel.md` for the publish/install runbook and `docs/superpowers/specs/2026-07-11-plugin-module-channel-design.md` for the full design.
