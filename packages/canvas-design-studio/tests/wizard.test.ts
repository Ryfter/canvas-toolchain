...
describe('formatSetupSummary', () => {
  it('includes the institution name in the heading', () => {
    expect(formatSetupSummary(baseConfig)).toContain('Example University');
  });
  ...
  it('shows Panopto domain when configured', () => {
    const config = {
      ...baseConfig,
      panopto: { domain: 'example.hosted.panopto.com', iframeWhitelisted: true as const },
    };
    expect(formatSetupSummary(config)).toContain('example.hosted.panopto.com');
  });

  it('includes the brand URL row when set', () => {
    const config = { ...baseConfig, brandUrl: 'https://www.example.edu/brand/' };
    expect(formatSetupSummary(config)).toContain('https://www.example.edu/brand/');
  });
