...
func NewWorkflowsScreen(parent fyne.Window, st *State, onNext, onBack func()) fyne.CanvasObject {
	...
	// Only the network fetch runs on this goroutine; all widget work (SetStatus,
	// checkbox construction, modulesBox.Add/Refresh) is marshalled onto the UI
	// thread with fyne.Do per Fyne v2.6's threading model (#122). The requested-
	// modules map stays mutex-guarded via State's requestedModule/
	// SetRequestedModule because the install step reads it from its own goroutine.
	go func() {
