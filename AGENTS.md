...
**Host config fan-out (pending merge; branch `feat/host-config-fanout`).** Installer now fans out the MCP config to all detected hosts (Claude Desktop/Code, Codex, Gemini CLI, Cursor, VS Code, Kiro, Antigravity) via a host adapter table in `installer/tasks/mcphost.go` (`SupportedHosts()` + format dispatcher). Sub-project A of the model-agnostic effort; see `installer/docs/specs/2026-06-27-host-config-fanout-design.md`.
