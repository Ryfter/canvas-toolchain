...
**Single professor-facing entrypoint:** the Command & Control MCP server. Professors talk to it from any MCP-capable AI client. The **installer auto-wires** Claude Desktop, Claude Code, Codex CLI, Gemini CLI, Cursor, VS Code, Kiro, and Antigravity (`installer/tasks/mcphost.go` `SupportedHosts()`). Any other MCP-capable client works via the manual JSON snippet — it is not auto-wired. Each underlying app stays independently usable.
...
`~/Dev/Grimdex` (this project's tier: `projects/canvas-toolchain/`).

- Read `~/Dev/Grimdex/GRIMDEX.md` FIRST — layout and contribution rules.
